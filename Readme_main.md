...
| 関数               | 関数は、**特定の仕事をしてくれる、小さなプログラムのかたまり**です。</br> 何かをお願い（データを渡す）すると、決まった処理を実行して、その結果を返してくれます。同じ作業を何度も繰り返す場合に、その作業を一つの関数としてまとめておくと、プログラムがとてもスッキリして分かりやすくなります。</br>Pythonでは以下のように関数を定義します。</br>`def 関数名(引数):`</br> `　　...`</br>`　　return 戻り値`                                                                                                                                                                                                                                                                                                                                                                                               |
| 関数の引数         | 関数に処理をお願いするときに渡す情報の箱です。</br> 例えば、`def load_yolo_model(model_path="yolo11n.pt")`の`model_path="yolo11n.pt"`は引数です。</br>`="yolov11n.pt"`は引数の初期値、すなわち箱に最初から入っているものです。</br>`load_yolo_model()`のように関数を使う時に何も指定しなければ関数はこの初期値を利用して処理をします。</br> `load_yolo_model("yolo11x.pt")`のように引数の指定があると、初期値の代わりに`"yolo11x.pt"`という値を`model_path`の箱に入れて関数内で使用します                                                                                                                                                                                                     |
| 関数の戻り値       | 戻り値は、関数が仕事を終えた後に**結果**として返してくれるものです。</br> この戻り値を受け取って、次の処理に使うことができます。</br>load_yolo_model 関数の場合は、以下の部分が戻り値に関わります。</br> `model = YOLO(model_path)`</br>`return model # <-  この 'return' の後に書かれたものが戻り値です`                                                                                                                                                                                                                                                                                                                                                                                     |
